...
from shapeformer.util.logger import log_visuals
...
class DataModule2(pl.LightningDataModule):
    def __init__(self, batch_size:int = 32, test_batch_size=None, val_batch_size=None, num_workers:int = 8, \
        ...
        for key in dset_opts:
            dset_opts[key] = copy.deepcopy(dset_opts[key]) # these dicts may share same fields, so let's replace them with their deepcopies 
