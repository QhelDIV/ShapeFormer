...
class PartNetDataset(Dataset):
    """ Modified from MSC """
    def __init__(self, split, data_root, category, n_pts, n_tg=32768, balanced_tg=False):
        super().__init__()
        if split == "validation":
            split = "val"
    ...
    def random_rm_parts(self, raw_pc, part_labels):
        ...
        part_ids_drop = part_ids[n_part_keep:]
        point_idx = []
        remain_idx= []
        for i in part_ids_keep:
            point_idx.extend(np.where(part_labels == i)[0].tolist())
        for i in part_ids_drop:
            remain_idx.extend(np.where(part_labels == i)[0].tolist())
        remained_pc = raw_pc.copy()[remain_idx]
        raw_pc = raw_pc[point_idx]
        return raw_pc, n_part_keep, remained_pc
    ...
    def __getitem__(self, index, **kwargs):
        ...
        raw_pc, n_part_keep, Xrm = self.random_rm_parts(real_pc, part_labels)
        raw_pc = sample_point_cloud_by_n(raw_pc, self.raw_n_pts)
        Xrm    = sample_point_cloud_by_n(Xrm,    self.raw_n_pts)
        ...
        if self.split=="train" and self.balanced_tg==True:
            ...
            Xtg       = Xtg[choice]
            Ytg       = Ytg[choice]
        return {"Xct": raw_pc, "Xbd": real_pc, "Xtg": Xtg, "Ytg": Ytg, "Xrm":Xrm,
...
class ProgPartNet(Dataset):
    def __init__(self, split, data_root, category, n_pts):
        super().__init__()
        if split == "validation":
            split = "val"
        self.phase = self.split = phase = split
