...
class ShapeFormer(pl.LightningModule):
    def __init__(self,  tuple_n=2, block_size=812, end_tokens=[4096, 4096], vocab_sizes=[4097, 4097], extra_vocab_sizes=[4097],
    ...
    def forward(self, **kwargs):
        ...
        c_indices, z_indices, extra_indices, others = self.representer.get_indices(
            **kwargs)
        cz_indices = torch.cat((c_indices, z_indices),
                               dim=1)  # (B, L_c+L_s, tuple_n)
    ...
    def sample_indices(self, c_indices, z_indices, max_steps, sample=False, best_in_first=False, top_k=100, top_p=.8, temperature=1.0,
        ...
        sampled[:, seq_head:seq_tail, :] = torch.cat(
            (c_indices, z_indices), dim=1)
        ...
        for j in sysutil.progbar(range(max_steps)):
            if seq_tail-seq_head >= block_size:
                ...
                seq_tail -= 1
            c_indices, z_indices = sampled[:, :L_c, :].contiguous(
            ), sampled[:, L_c:seq_tail, :].contiguous()
            idx = sampled[:, :seq_tail, :]
            extra_indices = self.representer.get_extra_indices(
                c_indices, z_indices)
            #print("extra ",extra_indices[0, L_c:seq_tail, 0])
            #print("idx ", idx[0, L_c:seq_tail, 0])
            #print("i", extra_indices)

            sample_gen = self.transformer.sample_next_tuple(
                idx, extra_idx=extra_indices, L_cond=L_c)
            # (B, vocab_size)
            ...
            for i in range(0, tuple_n):  # start from 1, self.tuple_n-1 items in total
                ...
                # new_ind: (B)
                new_ind = sample_logits(
                    logits, top_k=top_k, top_p=top_p, temperature=temperature, num_samples=1).reshape(-1)
                new_best_ind = sample_logits(
                    logits, top_k=1,     top_p=0.001, temperature=temperature, num_samples=1).reshape(-1)
            ...
            no_stop_token = (sampled[:, seq_tail-1, :]
                             != end_tokens[None, :]).all(axis=-1)
            if no_stop_token.long().sum() == 0:
                break
        # logits_history: [(B, vocab_size),]*tuple_n
        for i in range(0, tuple_n):
            logits_history[i] = rearrange(
                logits_history[i], "L B vocab_size -> B L vocab_size")
    ...
    def configure_optimizers(self):
        ...
        optim_groups = [
            {"params": [param_dict[pn] for pn in sorted(
                list(decay)) if pn in param_dict], "weight_decay": 0.01},
            {"params": [param_dict[pn] for pn in sorted(
                list(no_decay)) if pn in param_dict], "weight_decay": 0.0},
        ]
...
class VisShapeFormer(plutil.VisCallback):
    def __init__(self,  temperature=1, sample_n=10, top_k=300, top_p=.9, depth=5, decode_res=128,
                 sample_max_step=512, render_samples=64, end_tokens=None, mask_invalid=True, mask_invalid_completion=False,
    ...
    def visualize_batch(self, computed, input_name=""):
        ...
        for i in range(samples.shape[0]):
            imgs.update(vis_ind(indices=samples[i], prefix=f"s{i}", **config))
        ...
        for key in list(imgs.keys()):
            if "mesh" in key:
                ...
                del imgs[key]
                print("vf", vert.shape, face.shape)
                if vert.shape[0] < 10:
                    continue
