...
def raw2alpha(sigma, dist):
    ...
    T = torch.cumprod(torch.cat(
        [torch.ones(alpha.shape[0], 1).to(alpha.device), 1. - alpha + 1e-10], -1), -1)
...
class AlphaGridMask(torch.nn.Module):
    def __init__(self, device, aabb, alpha_volume):
        ...
        self.gridSize = torch.LongTensor(
            [alpha_volume.shape[-1], alpha_volume.shape[-2], alpha_volume.shape[-3]]).to(self.device)

    def sample_alpha(self, xyz_sampled):
        xyz_sampled = self.normalize_coord(xyz_sampled)
        alpha_vals = F.grid_sample(self.alpha_volume, xyz_sampled.view(
...
class TensorBase(torch.nn.Module):
    def __init__(self, aabb, gridSize, device=None, density_n_comp=8, appearance_n_comp=24, app_dim=27,
        ...
        self.init_render_func(shadingMode, pos_pe, view_pe,
                              fea_pe, featureC, device)
    ...
    def load(self, ckpt):
        if 'alphaMask.aabb' in ckpt.keys():
            ...
            self.alphaMask = AlphaGridMask(self.device, ckpt['alphaMask.aabb'].to(
                self.device), alpha_volume.float().to(self.device))
    ...
    def getDenseAlpha(self, gridSize=None):
        ...
        for i in range(gridSize[0]):
            alpha[i] = self.compute_alpha(
                dense_xyz[i].view(-1, 3), self.stepSize).view((gridSize[1], gridSize[2]))
    ...
    def updateAlphaMask(self, gridSize=(200, 200, 200)):
        ...
        alpha = F.max_pool3d(alpha, kernel_size=ks,
                             padding=ks // 2, stride=1).view(gridSize[::-1])
        ...
        for idx_chunk in idx_chunks:
            ...
            if bbox_only:
                ...
                mask_inbbox = (self.alphaMask.sample_alpha(
                    xyz_sampled).view(xyz_sampled.shape[:-1]) > 0).any(-1)
