...
class TensorVM(TensorBase):
    ...
    def compute_features(self, xyz_sampled):
        ...
        coordinate_plane = torch.stack(
            (xyz_sampled[..., self.matMode[0]], xyz_sampled[..., self.matMode[1]], xyz_sampled[..., self.matMode[2]])).detach()
        coordinate_line = torch.stack(
            (xyz_sampled[..., self.vecMode[0]], xyz_sampled[..., self.vecMode[1]], xyz_sampled[..., self.vecMode[2]]))
        coordinate_line = torch.stack(
            (torch.zeros_like(coordinate_line), coordinate_line), dim=-1).detach()
        ...
        plane_feats = F.grid_sample(
            self.plane_coef[:, :self.app_n_comp], coordinate_plane, align_corners=True).view(3 * self.app_n_comp, -1)
        line_feats = F.grid_sample(
            self.line_coef[:, :self.app_n_comp], coordinate_line, align_corners=True).view(3 * self.app_n_comp, -1)
    ...
    def compute_densityfeature(self, xyz_sampled):
        coordinate_plane = torch.stack((xyz_sampled[..., self.matMode[0]], xyz_sampled[...,
                                       self.matMode[1]], xyz_sampled[..., self.matMode[2]])).detach().view(3, -1, 1, 2)
        coordinate_line = torch.stack(
            (xyz_sampled[..., self.vecMode[0]], xyz_sampled[..., self.vecMode[1]], xyz_sampled[..., self.vecMode[2]]))
        coordinate_line = torch.stack((torch.zeros_like(
            coordinate_line), coordinate_line), dim=-1).detach().view(3, -1, 1, 2)
    ...
    def vectorDiffs(self, vector_comps):
        ...
        for idx in range(len(vector_comps)):
            ...
            dotp = torch.matmul(vector_comps[idx].view(
                n_comp, n_size), vector_comps[idx].view(n_comp, n_size).transpose(-1, -2))
    ...
    def upsample_volume_grid(self, res_target):
        ...
        plane_coef = F.interpolate(self.plane_coef.detach(
        ).data, scale_factor=scale, mode='bilinear', align_corners=True)
        line_coef = F.interpolate(self.line_coef.detach().data, size=(
            res_target[0], 1), mode='bilinear', align_corners=True)
...
class TensorVMSplit(TensorBase):
    ...
    def density_L1(self):
        ...
        for idx in range(len(self.density_plane)):
            ...
            total = total + torch.mean(torch.abs(self.density_plane[idx])) + torch.mean(
    ...
    def shrink(self, new_aabb):
        ...
        t_l, b_r = (xyz_min - self.aabb[0]) / \
            self.units, (xyz_max - self.aabb[0]) / self.units
        # print(new_aabb, self.aabb)
        # print(t_l, b_r,self.alphaMask.alpha_volume.shape)
        t_l, b_r = torch.round(torch.round(
            t_l)).long(), torch.round(b_r).long() + 1
...
class TensorCP(TensorBase):
    def __init__(self, aabb, gridSize, device=None, **kargs):
...
class TensorCP_pl(pl.LightningModule):
    def __init__(self, model_params={}, optim_opt=None):
        super().__init__()
        self.__dict__.update(locals())
        self.model = TensorCP(**model_params)

    def forward(self, **kwargs):
        return self.model(**kwargs)

    def get_loss(self, batch, batch_idx, stage="train"):
        out = self.forward(Xtg=batch["Xtg"])
        loss = torch.nn.functional.binary_cross_entropy_with_logits(
            input=out["logits"], label=batch["Ytg"])
        return {"loss": loss}
